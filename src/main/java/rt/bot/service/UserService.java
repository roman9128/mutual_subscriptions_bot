...
public class UserService {
    ...
    public void updateUser(BotUser botUser, ChannelTariff.Tariff tariff, ChannelTariff.ChosenPeriod period, BotUser.DialogStatus status) {
        botUser.setChosenTariff(tariff);
        botUser.setChosenPeriod(period);
        botUser.setDialogStatus(status);
        userRepository.save(botUser);
        log.info("Изменил пользователю с id {} тариф на {}, период на {} и статус на {}", botUser.getUserId(), tariff, period, status);
    }
}